...
The 40-test schema suite passes; ruff/black clean against the codified `pyproject.toml` lint config (ruff 0.7.4 + black 24.10.0, pinned to match `.pre-commit-config.yaml`). This document rationalizes the original three-vertical design (Insurance, Healthcare, HR). The Phase-4 vertical pivot has since shipped: `BusinessDocumentForm` (36 KILE fields, sourced from the upstream `rossumai/docile` `KILE_FIELDTYPES`) was added and is active alongside Healthcare; Insurance and HR remain in code as future-extensibility examples per the locked architectural decision but are not in the active cascade. None of this required changes to the shared constructs below — Section 13 records how the pivot landed.
...
In the complete V1 (local-first, locked 2026-05-14), the cascade has no cloud routing surface at all — the rules above are inert because every tier is local and no PHI ever enters the system. That the schema is *provider-agnostic* is a deliberate strength of the finished V1, not a placeholder: the same classification metadata would let the optional BAA-cloud enhancement (HIPAA-motivated; see `docs/hipaa-architecture.md`) drop in BAA-eligible AWS providers without touching any schema, where every provider in the routing table is BAA-eligible by design — local Tier 1 + Tier 2-local + Tier 3 (HIPAA-safe via the operator's environment) plus AWS Bedrock + Textract — and these rules become a startup-time defense-in-depth invariant rather than an active runtime restriction. The rules remain authoritative for the schema's data-classification intent regardless: if any deployment ever introduced a non-BAA provider (e.g., a cost-optimized non-BAA path for business documents in a non-healthcare deployment), the rules above describe how the routing layer should treat PHI/PCI fields against it. V1 operates against synthetic data only and never exercises the boundary live — which is exactly why it needs no BAA and is complete as-is.
...
```bash
# From the repo root (flat layout — intake_schemas.py is a top-level module).
just install                                      # uv sync + pre-commit
uv run python build_alias_seed.py                 # regenerates alias_table_seed.json
...
## 13. Schema scope: the Phase-4 vertical pivot (shipped)
