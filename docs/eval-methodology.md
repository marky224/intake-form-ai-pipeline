...
The static SVG F1-over-time chart that lands on V2's wake-page during Aurora cold-start (and on the README until Phase 6 produces real data) is generated by a separate `just chart` command that reads from `evals/results/` and emits to `docs/assets/f1-over-time.svg`. The chart is regenerated as a manual step at Phase 7 (V1 ships it as a portfolio artifact; V2 wires it into the wake-page) and again at Phase 10 polish; intermediate phases use the placeholder SVG.
