...
The 584-document corpus populates `train` (394 docs), but training pairs are built only from `train` docs carrying `seeded_correction` corrections, and the **corrections corpus itself is the deferred local piece** — no correction data is committed (only the 92-doc `test` split), so the manifest leakage guard still yields **0 non-leaky training pairs** at committed scale and the eval reports an **identity baseline (delta 0.000)**. That is the correct, publishable result: the reproducible pipeline + harness ship, and a real F1 delta requires a `FINETUNE_LIVE` run over the deferred local corrections corpus on the GPU box (the separate Phase 9 live-train follow-up). A "QLoRA on a small portfolio-scale correction corpus does not move the needle" finding is itself a credible result, not a failure.
