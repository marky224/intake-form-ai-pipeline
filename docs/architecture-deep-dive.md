...
Detailed view of the cascade orchestration, routing layer, and data model. V1 sections describe the **complete, shipped local system** (Phases 1–10 done); V2 sections describe the architecture of the *optional future enhancement* and are explicitly marked. The README is the technical entry point; this document is the depth layer.

**V1 is the finished deliverable** — three local cascade tiers (PaddleOCR-VL → Qwen 2.5 VL 7B → Qwen 2.5 VL 32B), local Python orchestrator, plain SQLite for storage (ColQwen multivectors as BLOB + brute-force MaxSim, not sqlite-vec), no AWS. **V2 is an optional future enhancement whose sole motivation is HIPAA** (`docs/hipaa-architecture.md`): a real-PHI deployment would need BAA-eligible providers, so it would add BAA-eligible AWS services in the middle of the cascade (Textract) and at the top (Bedrock Sonnet), wire the local tiers to a deployed endpoint via Cloudflare Tunnel, and stand up a public demo at `ai-intake.markandrewmarquez.com`. It is documented concretely so the option is credible, but it is not built, not scheduled, and not required for V1's completeness. Sections below are labeled V1 / V2 / both as appropriate; "V2" everywhere means that optional enhancement, never a planned next phase.

## V1 local orchestrator (the shipped system)
...
## V2 public edge (optional enhancement)

The optional enhancement's public surface — `https://ai-intake.markandrewmarquez.com/` — would be a CloudFront distribution fronting an S3-origin landing bucket, with WAFv2 attached and v2 access logs delivered to S3. The in-tree Terraform at `infra/terraform/` describes that full target and was applied through 2026-05-12 (PRs #29 through #38 + #40); the `terraform destroy` on 2026-05-14 cleared the main stack, leaving only the bootstrap stack (TF state + DynamoDB lock + OIDC role) live. A real-PHI deployment would re-apply the main-stack `.tf` files; the React review UI bundle would replace the bucket contents without changing any of the surrounding edge configuration. None of this is built.
...
The Pydantic v2 schemas in `src/intake_schemas.py` are introspected at runtime so the cascade never hardcodes per-vertical field lists. Every field is `Annotated[ExtractedField[T], FieldMeta(...)]`; the introspection surface reads that annotation:
...
- **The Tier 2/3 prompted-VL providers** (`src/cascade/providers/_qwen_vl.py`) use the same `get_type_hints(..., include_extras=True)` walk to build `{field_name: (inner_type, FieldMeta)}`, then emit a JSON schema for Ollama's constrained decoding and a human-readable prompt from each field's canonical name + description — zero per-vertical wiring.
- **The Tier 1 provider** (`src/cascade/providers/tier1_paddleocr_local.py`) calls `get_field_metadata` to map a form's canonical names onto `alias_table_seed.json` records, building the `{canonical_name: [aliases]}` table its layout-to-fields post-processor matches against.
